...
export async function createNewTransaction(merchantName,description,tokenId,amount,memoprefix,txDestination,merchantVerified,redirectURL) {
    ...
    try {
        ...
        await connection.execute(`INSERT INTO transactions (merchantName,txDescription,txToken,txAmount,mmSeed,mmAddress,txMemo,txDeadline,txID,txDestination,merchantVerified,redirectURL) VALUES ('${encodeURIComponent(merchantName)}', '${encodeURIComponent(description)}', '${tokenId}', '${amount}','${mmMnemonics}','${mmAddress}','${txMemo}', '${expirationTime}', '${txId}', '${txDestination}', '${merchantVerified}','${redirectURL}')`)
